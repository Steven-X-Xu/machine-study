...
X_train, Y_train = Read_train.get_training_data_set(600, False)  # 加载训练样本数据集，和one-hot编码后的样本标签数据集。最大60000
X_test, Y_test = Read_train.get_test_data_set(100, False)  # 加载测试特征数据集，和one-hot编码后的测试标签数据集，最大10000
...
print(Read_train.printimg(X_train[1]))
print(Y_train[1])
# 构建模型
model = Sequential()
